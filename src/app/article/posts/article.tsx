...
const postsDirectory = path.join(process.cwd(), "src/app/article/posts");
...
function findThumbnailFile(directory: string): string | null {
  ...
  return thumbnail ? path.join(directory, thumbnail) : null;
}

export async function toHTML(articles: {
  slug: string;
  title: string;
  date: string;
